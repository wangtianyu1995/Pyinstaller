...
def xcheck_host_prog(conf,name,tool,wafname=None):
	...
	if specific:
		...
		conf.env[wafname]+=value
		conf.msg('Will use cross-compilation %s from %s_%s'%(name,chost_envar,name)," ".join(quote(x)for x in value))
		return
	else:
		...
		if envar is not None:
			...
			conf.env[wafname]=value
			conf.msg('Will use cross-compilation %s from HOST_%s'%(name,name)," ".join(quote(x)for x in value))
			return
	if conf.env[wafname]:
		return
	value=None
